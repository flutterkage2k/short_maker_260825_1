...
- **내부망 전용 도구다. 인터넷에 공개된 서버에 올리지 마라.** 로그인·인증이 없고,
  삭제 API는 작업 폴더를 통째로 지운다. 외부에 노출하면 누구나 삭제·업로드가 가능하다.
- **Claude 유료 구독 + Claude Code CLI 로그인이 없으면 동작하지 않는다.**
  구간 선정과 업로드 정보 생성이 `claude` 명령을 호출하며, 사용량은 본인 구독에서 차감된다.
- **애플 실리콘 맥 전용이다.** 음성 인식(mlx-whisper)이 다른 플랫폼에서 안 돈다.
- **저작권은 사용자 책임이다.** 유튜브 다운로드는 YouTube 약관과 충돌할 수 있다.
  본인 영상 또는 권리를 확보한 영상에만 사용하라. 이 도구로 만든 결과물에 대한
  법적 책임은 전적으로 사용자에게 있다.
- **있는 그대로(as-is) 제공하며 지원하지 않는다.** 각자 환경에 맞춘 수정·재가공·문제
  해결은 각자의 몫이다. 라이선스는 MIT — 자유롭게 고쳐 쓰되 책임도 각자 진다.
